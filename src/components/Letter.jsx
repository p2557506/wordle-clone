...
const Letter = ({letterPos,attemptVal}) => {
  ...
  const {board,correctWord,currentAttempt,usedLetters, setUsedLetters ,almostLetters, setAlmostLetters,correctLetters,setCorrectLetters} = useContext(AppContext);
  const letter = board[attemptVal][letterPos];
  
  //Conditonal Styling for correct letters
  const correct = correctWord.toUpperCase()[letterPos] === letter //Accesse letter at position letterPos
  const almost = !correct && letter !== "" && correctWord.toUpperCase().includes(letter)//Letter is in word but wrong pos
